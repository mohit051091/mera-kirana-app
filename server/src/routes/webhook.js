...
router.post('/whatsapp', async (req, res) => {
    ...
    if (body.object) {
        if (body.entry &&
            ...
            try {
                if (res.rowCount === 0) console.log(`Duplicate msg (Background): ${messageId}`);
            }).catch(err => console.error("Async Logging Error:", err));

// 2. SESSION CHECK (FAST CACHE FIRST)
let isNewSession = true;
const now = Date.now();
const cached = sessionCache.get(from);

if (cached && (now - cached.lastChecked < CACHE_TTL)) {
    isNewSession = false; // User is already active in our memory
} else {
    // Only hit the DB if the user is not in our cache
    ...
    // Update Cache so the NEXT message is instant
    sessionCache.set(from, { isNewSession: false, lastChecked: now });
}

// 3. CUSTOMER METADATA (ASYNC)
// Ensuring customer exists or updating last_active status without blocking
...
if (isNewSession) {
    console.log(`Session Initialized (Cache: ${!isNewSession}) for ${from}`);
}
