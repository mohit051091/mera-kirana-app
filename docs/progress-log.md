...
## [2026-09-09] Slots/Express/Pre-order, Journey Trim, Bilingual Manual
- **Trigger:** Owner asked for rider-aware slots, express fee, scheduled discounts, shorter WhatsApp journey, owner manual (EN/HI), FSSAI frontend field; questioned Razorpay choice and .env provenance.
- **Action:** (1) sendSlotList rider-aware (capacity = AVAILABLE riders x rider_max_orders, legacy fallback; full slots auto-hide; riders-off shows tomorrow-only + note). (2) Express 10-min slot (+express_fee) gated on free rider; tomorrow slots show scheduled_discount% hint. (3) getSlotAdjustments helper charged identically in preview, confirm, and placement. (4) Removed CHOOSE_ORDER_TYPE screen (default ONE_TIME, subscribe upsell on bill); promptAddressStep helper (1-address skips to slots); address form prefills phone_number; address-save goes straight to slots. (5) migrate seeds rider_max_orders/express_fee/scheduled_discount; settings page gains Shop Identity (FSSAI+policy) and Rider/Express/Pre-order sections. (6) Bilingual owner manual (public .md EN/HI) + /guide page (toggle, downloads) + sidebar entry. (7) Confirmed server/.env holds TEST keys (rzp_test_); told owner live Razorpay keys needed for prod. Deployed SUCCESS + migrate re-run.

## [2026-09-09] All Railway Secrets Set (Voice, Razorpay Test, App Secret, Catalog Token)
- **Trigger:** Owner completed all 6 variable sets after service-pinned commands.
- **Action:** Verified live: 12/12 required keys present (SARVAM, GEMINI, RAZORPAY x3 test, WHATSAPP_APP_SECRET, META_CATALOG_TOKEN + prior 6). Deploy SUCCESS, /api/health OK/CONNECTED. NOTE: Razorpay keys are TEST (rzp_test_) � swap to live after KYC before opening online payments.
